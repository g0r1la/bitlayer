...
class KeyShuffler:
    def __init__(self,
                 seed: str = False) -> None:
        self.seed = seed
        # self.language = 'ru' if any(
        #     locale.getdefaultlocale()[0].startswith(lang) for lang in ['ru', 'be', 'kk', 'uk']) else 'en'
        self.language = 'en'
    ...
    def get_passphrase(self,
        ...
        if new:
            if self.language == 'ru':
                print('Придумайте секретную фразу/пароль (используйте больше 13 символов): ')
            else:
                print('Create and input your passphrase/password (use more than 13 characters): ')
        else:
            if self.language == 'ru':
                print('Введите вашу секретную фразу/пароль от приватников: ')
            else:
                print('Input your passphrase/password for private keys: ')
        self.seed = getpass.getpass('').strip()
    ...
    def encrypt_private_key(self,
        ...
        private_key = ''.join(
            char.upper() if i in last_two_indices else char for i, char in enumerate(private_key.lower()))
